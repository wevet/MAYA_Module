...
def compute_normal_map(gradient_x, gradient_y, intensity=1):
    ...
    if max_y > max_x:
        max_value = max_y
    normal_map = np.zeros((height, width, 3), dtype=np.float64)
...
def create(**kwargs):
    ...
    """
    sigma = kwargs.setdefault('smooth')
    intensity = kwargs.setdefault('intensity')
    input_file = kwargs.setdefault('input_file')
    output_file = kwargs.setdefault('output_file')
