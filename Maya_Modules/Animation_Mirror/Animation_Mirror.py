...
class Animation_Mirror:
    ...
    def _create_animation_window(self):
        ...
        cmds.window(WINDOW_NAME, bgc=(0.265, 0.265, 0.265), iconName="Animation Mirror",
                    title="Animation Mirror v1.0.0", h=400, s=0, w=280, nestedDockingEnabled=True)
        cmds.showWindow(WINDOW_NAME)
        cmds.window(WINDOW_NAME, h=300, s=0, e=1, w=280, resizeToFitChildren=1)
        cmds.frameLayout(cll=0, w=270, la="center", label="Animation Mirror Tool", lv=1, backgroundColor=(0, 0, 1),
                         backgroundShade=0)
        cmds.columnLayout("ThisCreateCol", ann="ThisCreateCol", bgc=(0.242, 0.242, 0.3), co=('both', 0), rs=2)
        cmds.columnLayout("InfoTextCol", columnAlign="center", co=('both', 5), rs=5, columnWidth=250, ann="InfoTextCol",
                          adjustableColumn=0)
        cmds.text(" ", h=2)
        ...
        cmds.checkBox("TranslationsCheckbox", en=1, cc=partial(self._change_options_mirror), ann="TranslationsCheckbox",
                      label="Translations", w=100, v=1)
        ...
        cmds.checkBox("RotationsCheckbox", en=1, cc=partial(self._change_options_mirror), ann="RotationsCheckbox",
                      label="Rotations", w=100, v=1)
        ...
        cmds.button("MirrorRealButton", enableBackground=1, command=partial(self._mirror_animation, False),
                    bgc=(0.25, 0.5, 0.4), h=30, ann="MirrorRealButton", label="Interactive Mirror", en=1, w=120)
        cmds.button("UndoMirrorButton", enableBackground=1, command=partial(self._delete_all_created),
                    bgc=(0.65, 0.5, 0.4), h=30, ann="UndoMirrorButton", label="Undo", en=1, w=70)
        cmds.button("BakeMirrorButton", enableBackground=1, command=partial(self._fast_bake, 1), bgc=(1, 0.5, 0.4),
                    h=30, ann="BakeMirrorButton", label="Bake", en=1, w=70)
        cmds.setParent('..')
        cmds.rowLayout("MirrorBakeRow", numberOfColumns=6, columnAttach=(1, "left", 5), ann="MirrorBakeRow")
        cmds.button("MirrorBakeButton", enableBackground=1, command=partial(self._mirror_animation, True),
                    bgc=(0.25, 0.5, 0.4), h=30, ann="MirrorBakeButton", label="Mirror with Bake", en=1, w=264)
        cmds.setParent('..')
    ...
    def _mirror_animation(self, with_bake: bool):
        ...
        if cmds.checkBox('RotationsCheckbox', q=True, v=True):
            referenceObjectRotate = cmds.xform(self.referenceObject[self.ind], query=True, rotation=True,
                                               objectSpace=True)
            ...
            if cmds.checkBox('ZAxisRotCheckbox', q=True, v=True):
                ...
                valZ = 1
            cmds.xform(self.mirrorObject[self.ind], ro=(
            (referenceObjectRotate[0] * valX), (referenceObjectRotate[1] * valY), (referenceObjectRotate[2] * valZ)),
                       os=1)
    ...
    def _transform_connect_operator(self, mirrorJoint, referenceJoint):
        ...
        if transX is True:
            ...
            cmds.connectAttr((str(constraint_point[0]) + ".constraintTranslateX"),
                             (self.operatorTrans[self.iTo] + ".floatA"), f=1)
        ...
        if transY is True:
            ...
            cmds.setAttr((self.operatorTrans[self.iTo] + ".floatB"), -1)
            cmds.setAttr((self.operatorTrans[self.iTo] + ".operation"), 2)
            cmds.connectAttr((str(constraint_point[0]) + ".constraintTranslateY"),
                             (self.operatorTrans[self.iTo] + ".floatA"), f=1)
        ...
        if transZ is True:
            ...
            cmds.setAttr((self.operatorTrans[self.iTo] + ".floatB"), -1)
            cmds.setAttr((self.operatorTrans[self.iTo] + ".operation"), 2)
            cmds.connectAttr((str(constraint_point[0]) + ".constraintTranslateZ"),
                             (self.operatorTrans[self.iTo] + ".floatA"), f=1)
    ...
    def _fast_bake(self, defineFrames: int):
        ...
        cmds.bakeResults(sparseAnimCurveBake=False, minimizeRotation=True, removeBakedAttributeFromLayer=False,
                         bakeOnOverrideLayer=False, preserveOutsideKeys=True, simulation=0, sampleBy=1,
                         time=(int(start), int(end)), disableImplicitControl=True)
        cmds.delete(sc=1)
