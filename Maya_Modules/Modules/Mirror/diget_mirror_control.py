...
class diget_mirror_control(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        self.left_ctrl_name_le.setPlaceholderText("Default: L, l, Lf")
        self.left_ctrl_name_le.setToolTip("左コントローラーのカスタム命名規則を入力する.\n" "By default works with 'L', 'l' and 'Lf'")
        self.right_ctrl_name_le = QtWidgets.QLineEdit()
        self.right_ctrl_name_le.setPlaceholderText("Default: R, r, Rt")
        self.right_ctrl_name_le.setToolTip("右コントローラーのカスタム命名規則を入力する.\n" "By default works with 'R', 'r' and 'Rt'")
    ...
    def create_layout(self):
        ...
        naming_convention_layout.addRow("Left Naming Scheme:", self.left_ctrl_name_le)
        naming_convention_layout.addRow("Right Naming Scheme:", self.right_ctrl_name_le)
    ...
    def get_vector_data(self, ctrl_list):
        """
        Description:
            全コントローラのベクトルを求め、dictionaryに格納する。
    ...
    def split_string(self, name, split):
        ...
        # Iterating over the lengt to get the correct amount of item in the list
        for i in range(lengt):
            string = ""
            index = None
            # Iterating over the split name to reassemple the name
            for x, item in enumerate(split_list):
                ...
                if not i == x and x != lengt:
                    string += split
            # In case the same letter is next to eachother
    ...
    def mirror_control(self):
        ...
        if operation == OperationType.left_to_right:
            ...
            if self.left_to_right_rb.isChecked():
                ...
                self.flip_frame(removed_left_ctrl_list, removed_right_ctrl_list, removed_middle_ctrl_list, data, pair_dict, mirror_axis, vector_data)
