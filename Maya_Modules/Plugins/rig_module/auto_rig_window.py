...
def create_auto_rig_window():
    ...
    cmds.paneLayout()
    cmds.textScrollList('ReRig_fkControlList', numberOfRows=8, allowMultiSelection=True, append=[], showIndexedItem=4)
    cmds.setParent('..')
    cmds.floatSliderGrp('ReRig_fkControlScale', field=True, label='Scale Fk Controles ', minValue=0.01, maxValue=5,
                        fieldMinValue=0.01, fieldMaxValue=20, value=1, precision=2, dragCommand=change_scale_fk_button,
    ...
    cmds.floatSliderGrp('reRig_heightWaistFK', field=True, label='Height for the FK contol ', minValue=0, maxValue=1,
...
def create_help_window(text):
    ...
    if text == 'biPedAutoRig':
        ...
        cmds.text(label='You can also choose how many joint you want in your rig and how high up you waist fk control',
                  font="plainLabelFont", align="left")
        cmds.text(label='will be. This will only be calculated if you have selected "spine".', font="plainLabelFont",
                  align="left")
...
def create_fk_controls_button():
    ...
    rig.create_fk_controls(jointList=cmds.ls(sl=True), lockAttributes=attr, constraint=const)


def parent_fk_controls_button():
    rig.parent_fk_controls(controlList=cmds.ls(sl=True))
    pass
...
def rename_joints_button():
    if cmds.radioCollection('ReRig_addToJoints', q=True, select=True) == 'ReRig_addToJoints_hierarchy':
        ...
        joint_list = cmds.ls(sl=True)
    rig.rename_joints(jointList=joint_list, add='_bn')


def create_main_groups_button():
    rig.create_main_groups()
    pass
...
def create_temporary_skeleton_button():
    ...
    rig.create_temporary_skeleton_spine()
    rig.create_temporary_skeleton_leg()
    cmds.select('c_neck_tempJt')
    rig.create_temporary_skeleton_arm()
    cmds.select('l_wrist_tempJt')
    rig.create_temporary_skeleton_fingers()
    ...
    temporary_skeleton_control = rig.create_temporary_skeleton_control(name='scale_tempSkeleton_ctrl')
...
def create_biped_rig_button():
    if not cmds.objExists('scale_tempSkeleton_ctrl'):
        ...
        if cmds.checkBox('reRig_autoRigSpine', q=True, value=True) is True:
            rig.create_main_groups()
            rig.create_spine_joint(scale=scale, side='c',
        ...
        if cmds.checkBox('reRig_autoRigArm', q=True, value=True) is True:
            rig.create_main_groups()
            rig.create_arm_joints(side='l', scale=scale)
        ...
        if ret_finger is True and cmds.checkBox('reRig_autoRigArm', q=True, value=True) is True:
            rig.create_main_groups()
            rig.create_finger_joints(scale=scale, side='l')
            rig.create_finger_joints(scale=scale, side='r')
        ...
        if ret_spine is True and cmds.checkBox('reRig_autoRigArm', q=True, value=True) is True:
            rig.connect_arm_to_spine()

        if cmds.checkBox('reRig_autoMasterCtrl', q=True, value=True) is True:
            rig.create_master_control(scale=scale)
            result_list.append('MASTER CONTROL')

        clean_string = rig.clean_list_tostring(dirtyList=result_list)
